class UsersController < ApplicationController
  ...
  def show
    ...
    @guest_reviews = Review.where(type: "GuestReview", host_id:current_user,guest_id: @user.id)

    # Display all the host reviews to host (if this user is a guest)
    @host_reviews = Review.where(type: "HostReview", guest_id:current_user ,host_id:@user.id)
  end
